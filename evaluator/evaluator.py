        ...
        if (predict_dir / 'result.json').exists():
            ...
            with open(groundtruth_dir / 'infos.json', 'r') as f:
                while line := f.readline():
                    ...
                    groundtruth[temp['ip']] = temp['PortService']
            ...
            with open(predict_dir / 'result.json', 'r') as f:
                ...
                while line := f.readline():
                    ...
                    for pS in groundtruth[temp['ip']]:
                        port, service = pS
                        if str(port) in temp['PortService']:
                            if service in temp['PortService'][str(port)]:
                                if predict_result.get(temp['ip'], None) is not None:
                                    predict_result[temp['ip']].append([port, service])
                                else:
                                    predict_result[temp['ip']] = [[port, service]]
                    for value in temp['PortService'].values():
                        total_predict_ports += len(value)
                    del temp

            for v in groundtruth.values():
                total_active_ports += len(v)
            for key, value in predict_result.items():
                positive_true_count += len(value)
                output_file.writelines(json.dumps({'ip':key, 'PortService':value}) + '\n')
    ...
    def evaluate(self):
        ...
        if self.positive_truth_output is not None:
            with open(self.positive_truth_output / utils.METRIC_FILENAME, 'a') as f:
                f.write(json.dumps({'total_active_services':total_active_ports, 'positive_true_count':positive_true_count,
                            'total_predict_services':total_predict_ports, 'coverage':coverage}) + '\n')
        
        print('total_active_services', total_active_ports)
        print('positive_true_count', positive_true_count)
        print('total_predict_services', total_predict_ports)
