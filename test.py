...
if True and pre:
    ...
    shutil.copyfile(index_to_service_path, model_path / 'index_to_service.json')
    shutil.copyfile(service_to_index_path, model_path / 'service_to_index.json')
...
saver = feature_extractor.extractor.FeatureSaver(model_path, extractor, subnet_threads=processes, chunk_size=chunk_size)
...
updater = Updater(combined_dir=input_path / 'subnet', positive_true_dir=positive_truth_path, model_dir=model_path / 'subnet',
                    ...
                    using_app_feature=app_feature, processes=processes, groundtruth_dir=groundtruth_path / 'subnet')
predictor = Predictor(input_path, model_path, predict_path, extractor_processes=processes, adders=adders,
                    processes=processes, predict_port_num=service_num_per_cycle, uncertainty_weigh=0, chunk_size=chunk_size)
